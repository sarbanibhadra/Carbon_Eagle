...
const updatePointChart = (point_data) => {
  ...
  const x_day = point_data.day
  const daily_concentration = point_data.daily_concentration

  const percentage_exposure = point_data.percentage_exposure
  ...
  chart_daily_concentration.load({
    columns: [
      ['x'].concat(x_day),
      ['data'].concat(daily_concentration)
    ],
  });

  gauge_exposure.load({
    columns: [['% days above WHO guideline value', percentage_exposure]]
  });
